...
def test_api_meteocat_count_lightnings_id_00(api, postgresql_schema):
    """
    Tests same day that has some data in the database.
    ...
    response = api.get('/v1/meteocat/lightning/discharge_count/1151242', headers=headers)
    assert response.content_type == 'application/json'
    assert response.status_code == 200
    data = json.loads(response.get_data(as_text=True))
    assert len(data) == 1
    ...
    assert record[1] == 3
    assert record[2] == '127.0.0.1'
    assert record[3] == 'http://localhost/v1/meteocat/lightning/discharge_count/1151242'
    assert record[4] == 'GET'
    assert record[5] == '{}'
    assert record[6] == 200
