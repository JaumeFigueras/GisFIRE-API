...
def test_empty_api_meteocat_stations_entrypoint_01(api, postgresql_schema):
    ...
    count = record[0]
    response = api.get('/v1/meteocat/stations/')
    ...
    assert record[3] == 'http://localhost/v1/meteocat/stations/'
    assert record[4] == 'GET'
    assert record[5] == '{}'
    assert record[6] == 500
...
def test_empty_api_meteocat_stations_nearest_parameters_01(api, postgresql_schema):
    ...
    response = api.get('/v1/meteocat/stations/nearest/', headers=headers)
    assert response.content_type == 'application/json'
    ...
    assert record[1] == 3
    assert record[2] == '127.0.0.1'
    assert record[3] == 'http://localhost/v1/meteocat/stations/nearest/'
    assert record[4] == 'GET'
    assert record[5] == '{}'
    assert record[6] == 422
...
def test_empty_api_meteocat_stations_nearest_parameters_02(api, postgresql_schema):
    ...
    count = record[0]
    headers = {'Authorization': 'Basic {}'.format(b64encode(b"jack.skellington:0123456789asdfghjkl").decode("utf-8"))}
    response = api.get('/v1/meteocat/stations/nearest?a=1&b=2&c=3&d=4&e=5', headers=headers)
    assert response.content_type == 'application/json'
    assert response.status_code == 422
    data = json.loads(response.get_data(as_text=True))
    assert len(data) == 1
    ...
    assert record[1] == 3
    assert record[2] == '127.0.0.1'
    assert record[3] == 'http://localhost/v1/meteocat/stations/nearest?a=1&b=2&c=3&d=4&e=5'
    assert record[4] == 'GET'
...
def test_empty_api_meteocat_stations_nearest_parameters_03(api, postgresql_schema):
    ...
    count = record[0]
    headers = {'Authorization': 'Basic {}'.format(b64encode(b"jack.skellington:0123456789asdfghjkl").decode("utf-8"))}
    response = api.get('/v1/meteocat/stations/nearest?a=1&b=2&c=3', headers=headers)
    assert response.content_type == 'application/json'
    assert response.status_code == 422
    data = json.loads(response.get_data(as_text=True))
    assert len(data) == 1
    ...
    assert record[1] == 3
    assert record[2] == '127.0.0.1'
    assert record[3] == 'http://localhost/v1/meteocat/stations/nearest?a=1&b=2&c=3'
    assert record[4] == 'GET'
...
def test_empty_api_meteocat_stations_nearest_parameters_04(api, postgresql_schema):
    ...
    count = record[0]
    headers = {'Authorization': 'Basic {}'.format(b64encode(b"jack.skellington:0123456789asdfghjkl").decode("utf-8"))}
    response = api.get('/v1/meteocat/stations/nearest?a=1&b=2&c=3&d=4', headers=headers)
    assert response.content_type == 'application/json'
    assert response.status_code == 422
    data = json.loads(response.get_data(as_text=True))
    assert len(data) == 1
    ...
    assert record[1] == 3
    assert record[2] == '127.0.0.1'
    assert record[3] == 'http://localhost/v1/meteocat/stations/nearest?a=1&b=2&c=3&d=4'
    assert record[4] == 'GET'
...
def test_empty_api_meteocat_stations_nearest_parameters_05(api, postgresql_schema):
    ...
    count = record[0]
    headers = {'Authorization': 'Basic {}'.format(b64encode(b"jack.skellington:0123456789asdfghjkl").decode("utf-8"))}
    response = api.get('/v1/meteocat/stations/nearest?date=2021-11-01T22:00:00Z&srid=4258&c=3&d=4', headers=headers)
    assert response.content_type == 'application/json'
    assert response.status_code == 422
    data = json.loads(response.get_data(as_text=True))
    assert len(data) == 1
    ...
    assert record[1] == 3
    assert record[2] == '127.0.0.1'
    assert record[3] == 'http://localhost/v1/meteocat/stations/nearest?date=2021-11-01T22:00:00Z&srid=4258&c=3&d=4'
    assert record[4] == 'GET'
...
def test_empty_api_meteocat_stations_nearest_parameters_06(api, postgresql_schema):
    ...
    count = record[0]
    headers = {'Authorization': 'Basic {}'.format(b64encode(b"jack.skellington:0123456789asdfghjkl").decode("utf-8"))}
    response = api.get('/v1/meteocat/stations/nearest?date=2021-11-01T22:00:00Z&srid=4258&x=3.25&lat=4.58',
                       headers=headers)
    assert response.content_type == 'application/json'
    assert response.status_code == 422
    data = json.loads(response.get_data(as_text=True))
    ...
    assert record[1] == 3
    assert record[2] == '127.0.0.1'
    assert record[3] == 'http://localhost/v1/meteocat/stations/nearest?date=2021-11-01T22:00:00Z&srid=4258&x=3.25&' \
                        'lat=4.58'
    assert record[4] == 'GET'
...
def test_empty_api_meteocat_stations_nearest_parameters_07(api, postgresql_schema):
    ...
    count = record[0]
    headers = {'Authorization': 'Basic {}'.format(b64encode(b"jack.skellington:0123456789asdfghjkl").decode("utf-8"))}
    response = api.get('/v1/meteocat/stations/nearest?date=2021-11-01T22:00:00Z&srid=4258&x=3.25&lon=4.58',
                       headers=headers)
    assert response.content_type == 'application/json'
    assert response.status_code == 422
    data = json.loads(response.get_data(as_text=True))
...
def test_empty_api_meteocat_stations_nearest_parameters_08(api, postgresql_schema):
    ...
    count = record[0]
    headers = {'Authorization': 'Basic {}'.format(b64encode(b"jack.skellington:0123456789asdfghjkl").decode("utf-8"))}
    response = api.get('/v1/meteocat/stations/nearest?date=2021-13-01T22:05:37Z&srid=4258&lon=2.33371&lat=41.71408',
                       headers=headers)
    assert response.content_type == 'application/json'
    ...
    assert record[1] == 3
    assert record[2] == '127.0.0.1'
    assert record[3] == 'http://localhost/v1/meteocat/stations/nearest?date=2021-13-01T22:05:37Z&srid=4258&' \
...
def test_empty_api_meteocat_stations_nearest_parameters_09(api, postgresql_schema):
    ...
    count = record[0]
    headers = {'Authorization': 'Basic {}'.format(b64encode(b"jack.skellington:0123456789asdfghjkl").decode("utf-8"))}
    response = api.get('/v1/meteocat/stations/nearest?date=2021-11-01T22:05:37Z&srid=4258a&lon=2.33371&lat=41.71408',
                       headers=headers)
    assert response.content_type == 'application/json'
    assert response.status_code == 400
    data = json.loads(response.get_data(as_text=True))
    ...
    assert record[1] == 3
    assert record[2] == '127.0.0.1'
    assert record[3] == 'http://localhost/v1/meteocat/stations/nearest?date=2021-11-01T22:05:37Z&srid=4258a&' \
                        'lon=2.33371&lat=41.71408'
    assert record[4] == 'GET'
...
def test_empty_api_meteocat_stations_nearest_parameters_10(api, postgresql_schema):
    ...
    count = record[0]
    headers = {'Authorization': 'Basic {}'.format(b64encode(b"jack.skellington:0123456789asdfghjkl").decode("utf-8"))}
    response = api.get('/v1/meteocat/stations/nearest?date=2021-11-01T22:05:37Z&srid=4258&lon=2.33371&lat=41a.71408',
                       headers=headers)
    assert response.content_type == 'application/json'
    assert response.status_code == 400
    data = json.loads(response.get_data(as_text=True))
...
def test_empty_api_meteocat_stations_nearest_parameters_11(api, postgresql_schema):
    ...
    count = record[0]
    headers = {'Authorization': 'Basic {}'.format(b64encode(b"jack.skellington:0123456789asdfghjkl").decode("utf-8"))}
    response = api.get('/v1/meteocat/stations/nearest?date=2021-11-01T22:05:37Z&srid=4258&lon=2b.33371&lat=41.71408',
                       headers=headers)
    assert response.content_type == 'application/json'
    assert response.status_code == 400
    data = json.loads(response.get_data(as_text=True))
